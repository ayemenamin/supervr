...
class PosController extends Controller
{
    public function index() {
        ...
        $query = ProductOrder::where('completed', 'no');
    ...
    public function placeOrder(Request $request) {
        ...
        if(empty($request->waiter_id)){
            $waiter_id_isert = Auth::user()->id;
        }else{
          $waiter_id_isert = $request->waiter_id;
        }
        $po->admin_id = $waiter_id_isert;

        $po->save();
        $table = Table::where('table_no',  $request->table_no)->first();

        // Update the is_busy field of the table to 1
        if ($table) {
            $table->is_busy = 1;
            $table->save();
        }
